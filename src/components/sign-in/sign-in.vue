...
import { useAuthService } from '@/util/auth/auth.service';
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleLogin = () => {
      authService.tryLogin(email.value, password.value).then(() => {
        router.push('/');
      }).catch((error) => {
        alert('Login failed');
      });
    ...
    const handleRegister = () => {
      authService.tryRegister(registerEmail.value, registerPassword.value).then(() => {
        toggleCard();
      }).catch((err) => {
        alert(err.message);
      });
